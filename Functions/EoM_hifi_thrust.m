function dXdt = EoM_hifi_thrust(t, X0, Tmax, D, Isp, N_act, g0, ubig, mu_central, N_perts, mu_perts, x_centPertBig, y_centPertBig, z_centPertBig, times, uTimes, TU)
...
% Get control vector at current time by splining larger array
% ux = interp1(uTimes, ubig(1,:), t*TU);
% uy = interp1(uTimes, ubig(2,:), t*TU);
% uz = interp1(uTimes, ubig(3,:), t*TU);
lower = find(uTimes <= ty, 1, 'last');
higher = find(uTimes >= ty, 1);
if ty > uTimes(end)
    higher = length(uTimes);
    lower = length(uTimes)-1;
elseif ty < uTimes(1)
    lower = 1;
    higher = 2;
elseif ty == uTimes(higher) || ty == uTimes(lower)
    if lower ~= 1
        lower = lower - 1;
    elseif higher ~= length(uTimes)
        higher = higher + 1;
    end
end
t1 = uTimes(higher);
...
uxl = ubig(1,lower);
uxu = ubig(1,higher);
ux = uxl + (t1 - t0)*((uxu - uxl)/(t1 - t0));
uyl = ubig(2,lower);
uyu = ubig(2,higher);
uy = uyl + (t1 - t0)*((uyu - uyl)/(t1 - t0));
uzl = ubig(3,lower);
uzu = ubig(3,higher);
uz = uzl + (t1 - t0)*((uzu - uzl)/(t1 - t0));
u = [ux; uy; uz];

% Thrust magnitude
dm = (Tmax*D*norm(u))/(Isp*g0);
T =  N_act*D*Tmax*u;
...
dXdt(2) = X0(5);
dXdt(3) = X0(6);
dXdt(4) = -(mu_central*r(1))/norm_r3 + sum(dv_pert(1,:)) + T(1)/X0(7);
dXdt(5) = -(mu_central*r(2))/norm_r3 + sum(dv_pert(2,:)) + T(2)/X0(7);
dXdt(6) = -(mu_central*r(3))/norm_r3 + sum(dv_pert(3,:)) + T(3)/X0(7);
dXdt(7) = -dm;
dXdt = dXdt.';
