function [c, ceq] = con_hifi(x,consts)
...
noCeqs = 7*NpCurrent + flybyPresent;
...
% noCs = thrustLengthF + thrustLengthB + flybyPresent
...
%     indThrust = whichThrust(i);
% %     u_this = u((1:3)+(i-1)*3,:);
%     u_this = [uxFall(indThrust, 1:lengthF(indThrust)); uyFall(indThrust, 1:lengthF(indThrust)); uzFall(indThrust, 1:lengthF(indThrust))];
%     c(1+indCNow : indCNow + lengthF(indThrust)) = vecnorm(u_this,2,1)-1;
%      indCNow = indCNow + lengthF(indThrust);
%      
%     u_this = [uxBall(indThrust, 1:lengthB(indThrust)); uyBall(indThrust, 1:lengthB(indThrust)); uzBall(indThrust, 1:lengthB(indThrust))];
%     c(1+indCNow : indCNow + lengthB(indThrust)) = vecnorm(u_this,2,1)-1;
%      indCNow = indCNow + lengthB(indThrust);
% %     cInd = N*i;
% end
for i = 1:thrustPresent
    u_this = u((1:3)+(i-1)*3,:);
...
for i = 1:flybyPresent
    if FBEnd(i) == 1
        ...
        c(cInd+1) = rp_all(i) - h_mins(i) - r_flybys(i);
        ceqInd = ceqInd+2;
    end
