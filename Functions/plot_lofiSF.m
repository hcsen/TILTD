function [c, ceq] = plot_lofiSF(x,consts)
...
for i = 1:NpCurrent
    ...
    plot3(x_all(i, end)*DU, y_all(i, end)*DU, z_all(i, end)*DU, 'bx')
end
...
for i = 1:NpCurrent
    ...
    plot(t_all(i,1:N/2+2)*TU, vecnorm(v(:,1:N/2+2), 2,1)*VU, 'k-')
    plot(t_all(i,N/2+3:end)*TU, vecnorm(v(:,N/2+3:end), 2,1)*VU, 'k-')
    plot(t_all(i,end)*TU, vecnorm(v(:,end), 2,1)*VU, 'kx')
end
...
if N_flybys > 0
    for i = 1:flybyPresent
        ...
        c(cInd+1) =  r_flybys(i) + rp_all(i) - h_maxs(i);
        cInd = cInd + 2;
    end
end
% disp(rp_all*DU)
