...
rng(1);     % To prevent using the same random seed every run
...
MBH_noLoops = 2;                    % Number of times to run
...
%% No more user inputs, skip to setting file names at the end
% Ephemeris data
...
dt_all(Np) = (dt_max(i) - dt_min(i))*rand + dt_min(i);
...
if Np > 2
    for i = 2:Np-1
        ...
        if any(i == whichFlyby)
            ...
            ind_vinfi(:,i) = (length(x) - 5):1:(length(x)-3);
            ind_vinff(:,i) = (length(x) - 2):1:length(x);
...
dlmwrite('TEE_optim_archive_2.csv', optim_archive, 'delimiter', ',', 'precision', 20);
dlmwrite('TEE_consts_2.csv', consts, 'delimiter', ',', 'precision', 20);
dlmwrite('TEE_viol_2.csv', violation_archive, 'delimiter', ',', 'precision', 20);
dlmwrite('TEE_lb_2.csv', lb, 'delimiter', ',', 'precision', 20);
dlmwrite('TEE_ub_2.csv', ub, 'delimiter', ',', 'precision', 20);